...
all.run.set <- read.table("Figure_S7a_MyxoGeoInfor.txt",header = T,sep = '\t')
Myxo.yes <- all.run.set[which(all.run.set$percentReads >= 0.001),]
...
p + geom_point(data = Myxo.yes,
  ...
  scale_size_discrete(labels = c("0.1","1","3","7","10"))+
...
ggsave("Figure_S7a_MyxoBiodistribution_raw.pdf", units = "in",width = 7.27, height = 4.8)

# save config
# 7.27 * 4.8 in
# following modified by PhantomPDF (Foxit) software

summary(Myxo.yes$EnvirTypes)
summary(all.run.set$EnvirTypes)

names(summary(Myxo.yes$EnvirTypes))
names(summary(all.run.set$EnvirTypes))

percentSample <- paste0(summary(Myxo.yes$EnvirTypes), 
                        "/", 
                        summary(all.run.set$EnvirTypes))
names(percentSample) <- names(summary(Myxo.yes$EnvirTypes))
percentSample
sum(summary(Myxo.yes$EnvirTypes))
